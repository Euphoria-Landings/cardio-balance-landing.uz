...
export default function OrderModal({ isOpen, onClose }: OrderModalProps) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-[#1A1A1A]/80 backdrop-blur-xl">
        ...
        <div className="relative w-full max-w-[500px] bg-white rounded-[50px] shadow-[0_50px_100px_rgba(0,0,0,0.4)] overflow-hidden">
              ...
              <div>
                <div className="text-center mb-10">
                  <h2 className="text-4xl font-black italic tracking-tighter text-[#1A1A1A]">
                    CARDIO <span className="text-red-600">BALANCE</span>
                  </h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div
                    className={`bg-gray-50 border-2 rounded-3xl transition-colors ${
                      activeField === "name"
                  ...
                  >
                    <div className="relative flex items-center">
                      ...
                      <input
                        ...
                        className="w-full px-4 py-5 bg-transparent outline-none font-bold text-[#1A1A1A]"
                        value={formData.name}
                        onChange={(e) =>
                  ...
                  <button
                    ...
                    className="w-full py-6 bg-red-600 rounded-3xl text-white font-black uppercase tracking-[3px] text-xs shadow-2xl transition-all active:scale-95 disabled:bg-gray-200 disabled:shadow-none"
                  >
                    {status === "loading"
                      ? "Yuborilmoqda..."
                      : "Buyurtma berish"}
                ...
                <p className="text-center text-[9px] text-gray-400 mt-8 leading-relaxed px-4 uppercase font-bold tracking-widest">
                  Xavfsiz va Tabiiy Mahsulot
                </p>
              </div>
            )}
          </div>
          ...
          <button
            onClick={onClose}
            className="absolute top-6 right-6 w-10 h-10 flex items-center justify-center rounded-full bg-gray-50 text-gray-400 hover:text-red-600 transition-all"
          >
            <svg
              width="20"
              height="20"
